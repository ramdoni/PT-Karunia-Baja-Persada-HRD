...
{
    use WithPagination;
    public $keyword,$type,$reas,$status;
    protected $total;
    ...
    public function render()
    {
        ...
        if(!$this->reas){
            $data = $data->where(function($table){
                $table->where('reference_type','Endorsement DN')
                        ->orWhere('reference_type','Endorsement DN Reas');
                });
        }else{
        ...
        return view('livewire.endorsement.dn')->with(['data'=>$data->paginate(100),'total_payment_amount'=>format_idr($total->sum('payment_amount'))]);
    }

    public function delete($id)
    {
        \App\Models\Income::find($id)->delete();
    }
    ...
    public function downloadExcel()
    {
        ...
        $objPHPExcel->getProperties()->setCreator("Stalavista System")
                                    ...
                                    ->setCategory("Income");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Endorsement Debit Note');
        ...
        },'endorsement-debit-note-' .date('d-M-Y') .'.xlsx');
    }
}