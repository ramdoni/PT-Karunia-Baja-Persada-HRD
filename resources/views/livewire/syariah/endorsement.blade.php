<div class="mt-2" id="keydown">
    <div class="row">
        ...
        <div class="col-md-3">
            <a href="javascript:void(0)" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#modal_upload_endorsement" class="mb-2 btn btn-info" style="width:150px;"><i class="fa fa-upload"></i> Upload</a>
            @if($total_sync>0)
            <a href="javascript:void(0)" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#modal_confirm_sync_endorsement" class="mb-2 btn btn-warning"><i class="fa fa-refresh"></i> Sync {{$total_sync?"(".$total_sync.")" : "(0)"}}</a>
            @endif
        </div>
    ...
    <div wire:ignore.self class="modal fade" id="modal_upload_endorsement" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <livewire:syariah.endorsement-upload>
            </div>
        </div>
    </div>
    <div class="modal fade" id="modal_confirm_sync_endorsement" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <livewire:syariah.endorsement-sync>
            </div>
        </div>
    </div>
    <div class="modal fade" id="modal_check_data_endorsement" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="max-width:90%;" role="document">
            <div class="modal-content">
                <livewire:syariah.endorsement-check-data>
            </div>
        </div>
    </div>
</div>
...
<script>
    Livewire.on('refresh-page',()=>{
        $("#modal_upload_endorsement").modal('hide');
        $("#modal_check_data_endorsement").modal('hide');
        $("#modal_confirm_sync_endorsement").modal('hide');
    });
    Livewire.on('emit-check-data-endorsement',()=>{
        $("#modal_upload_endorsement").modal("hide");
        setTimeout(function(){
            $("#modal_check_data_endorsement").modal(
                {
                    backdrop: 'static',
                    keyboard: false
                });
