<div class="body">
    <div class="row">
        ...
        <div>
            <a href="javascript:void(0)" class="btn btn-info btn-sm" wire:click="emit('confirm-replace-all-refund')"><i class="fa fa-refresh"></i> Replace All</a>
            <a href="javascript:void(0)" class="btn btn-success btn-sm" wire:click="emit('confirm-keep-all-refund')"><i class="fa fa-check"></i> Keep All</a>
            <a href="javascript:void(0)" class="btn btn-danger btn-sm" wire:click="$emit('confirm-delete-all-refund')"><i class="fa fa-trash"></i> Delete All</a>
        </div>
        <div wire:loading>
        ...
        <table class="table m-b-0 table-hover c_list">
            ...
            <tbody>
                ...
                <tr>
                    <td rowspan="2">{{$num}}</td>
                    <td>
                        <a href="javascript:;" wire:click="$emit('replace-confirm-refund',{{$item->id}})" class="text-info"><i class="fa fa-refresh"></i></a>
                        <a href="javascript:;" wire:click="$emit('delete-confirm-refund',{{$item->id}})" class="text-danger ml-1"><i class="fa fa-trash"></i></a>
                        <a href="javascript:;" wire:click="$emit('keep-confirm-refund',{{$item->id}})" class="text-info ml-1" title="Keep Data"><i class="fa fa-check"></i></a>
                    </td>
                    <td>
...
@push('after-scripts')
<script>
Livewire.on('confirm-replace-all-refund',()=>{
    if(confirm('Replace all data ?')){
        Livewire.emit('replace-all-refund');
    }
});
Livewire.on('confirm-delete-all-refund',()=>{
    if(confirm('Delete all data ?')){
        Livewire.emit('delete-all-refund');
    }
});
Livewire.on('confirm-keep-all-refund',()=>{
    if(confirm('Keep all data ?')){
        Livewire.emit('keep-all-refund');
    }
});
Livewire.on('keep-confirm-refund',(id)=>{
    if(confirm('Keep this data ?')){
        Livewire.emit('keep-refund',id);
    }
});
Livewire.on('delete-confirm-refund',(id)=>{
    if(confirm('Delete this data ?')){
        Livewire.emit('delete-refund',id);
    }
});
Livewire.on('replace-confirm-refund',(id)=>{
    if(confirm('Replace this data ?')){
        Livewire.emit('replace-refund',id);
    }
});
</script>
@endpush